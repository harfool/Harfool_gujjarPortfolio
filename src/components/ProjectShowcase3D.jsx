...
/**
 * Optimized Cinematic Project Card with 3D Tilt & Glassmorphism
 * - Memoized to prevent unnecessary re-renders
 * - useCallback for event handlers
 * - Reduced motion support
 */
const ProjectCard3D = memo(({ project, onClick, index }) => {
...
/**
 * Optimized Cinematic Project Modal - Mini Case Study
 * - Memoized to prevent re-renders on parent state changes
 * - useCallback for onClose handler
 */
const ProjectModal3D = memo(({ project, isOpen, onClose }) => {
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <>
          {/* Blurred Glass Backdrop - Hidden on mobile */}
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            ...
            onClick={onClose}
            className="hidden md:block fixed inset-0 bg-black/80 backdrop-blur-2xl z-50 cursor-pointer overflow-hidden"
          />

          {/* Modal Container - Hidden on mobile, visible on tablet+ */}
          <div className="hidden md:flex fixed inset-0 z-50 items-center justify-center pointer-events-none overflow-hidden p-6">
            <motion.div
              initial={{
                opacity: 0,
                scale: 0.85,
              ...
              className="relative w-[75vw] h-[75vh] lg:w-[50vw] lg:h-[50vh] bg-gradient-to-br from-slate-950/95 via-slate-900/90 to-slate-950/95 backdrop-blur-3xl rounded-3xl shadow-2xl border border-slate-700/50 pointer-events-auto flex flex-col"
              onClick={(e) => e.stopPropagation()}
            >
              ...
              <div className="flex-1 overflow-y-auto p-8 space-y-6" style={{scrollbarWidth: 'thin', scrollbarColor: '#8b5cf6 transparent'}}>
                {/* Project Name - Title */}
                <motion.div
...
ProjectModal3D.displayName = 'ProjectModal3D';

/**
 * Main Optimized Project Showcase Component
 * - useMemo for filtered projects to prevent recalculation
...
export default function ProjectShowcase3D({ projects }) {
  ...
  const categories = useMemo(() => ['all', 'featured', 'web', 'mobile', 'fullstack'], []);
